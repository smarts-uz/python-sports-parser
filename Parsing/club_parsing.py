...
# Function to create a Club only if it doesn't exist
def create_club_if_not_exists(name, **fields):
    # Klubni qidirish
    club_exists = Club.objects.filter(name=name).exists()

    if club_exists:
        # Agar klub mavjud bo'lsa, bu jarayonni davom ettiradi
        print(f"Club '{name}' already exists, skipping creation.")
        return None, False

    # Agar klub mavjud bo'lmasa, yangi klub yaratadi
    club = Club.objects.create(name=name, **fields)
    return club, True
...
def parse_clubs_for_competition(competition):
    ...
    for row in table_rows:
        ...
        club, created = create_club_if_not_exists(
            name=name_en,
            flag_url=relative_logo_path,
            country_id=competition.country_id,
            name_ru=name_ru,
        ...
        if club:  # Check if club instance was created
            if created:
                print(f'{name_en} created successfully. Club ID: {club.id}, Club Link: {club_link}')
